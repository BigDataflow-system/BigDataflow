...
public final class Tools {
    public static Stmt newStmt(Scanner sc) {
        Stmt stmt = null;
        String type = sc.next();
        if (type.equals("assign")) {
            stmt = new AssignStmt(sc);
        }
        else if (type.equals("load")) {
            stmt = new LoadStmt(sc);
        }
        else if (type.equals("store")) {
            stmt = new StoreStmt(sc);
        }
        else if (type.equals("alloca")) {
            stmt = new AllocStmt(sc);
        }
        else if (type.equals("phi")) {
            stmt = new PhiStmt(sc);
        }
        else if (type.equals("call")) {
            stmt = new CallStmt(sc);
        }
        else if (type.equals("return")) {
            stmt = new ReturnStmt(sc);
        }
            ...
            stmt = new CallfptrStmt(sc);
        }
        else if(type.equals("calleefptr")){
            stmt = new CalleefptrStmt(sc);
        }
        else {
    ...
    public static boolean propagate(Pegraph oldPEG, Pegraph newPEG) {
        return !newPEG.equals(oldPEG);
    }

    public static Pegraph transfer(Pegraph pegraph, Stmt stmt, Grammar grammar, Singletons singletons) {
        Pegraph out_pegraph = pegraph.getNew();
        switch (stmt.getType()) {
            case Assign:
                transfer_copy(out_pegraph, (AssignStmt)stmt, grammar, singletons);
                break;
            case Load:
                transfer_load(out_pegraph, (LoadStmt)stmt, grammar, singletons);
                break;
            case Store:
                transfer_store(out_pegraph, (StoreStmt)stmt, grammar, singletons);
                break;
            case Alloca:
                transfer_address(out_pegraph, (AllocStmt)stmt, grammar, singletons);
                break;
            case Phi:
                transfer_phi(out_pegraph, (PhiStmt)stmt, grammar, singletons);
                break;
            case Call:
                transfer_call(out_pegraph, (CallStmt)stmt, grammar, singletons);
                break;
            case Return:
                transfer_return(out_pegraph, (ReturnStmt)stmt, grammar, singletons);
                break;
            case Ret:
                transfer_ret(out_pegraph, (RetStmt)stmt, grammar, singletons);
                break;
            case Skip:
                transfer_skip(out_pegraph, (SkipStmt)stmt, grammar, singletons);
                break;
            case Callfptr:
                transfer_callfptr(out_pegraph, (CallfptrStmt)stmt, grammar, singletons);
                break;
            case Calleefptr:
                transfer_calleefptr(out_pegraph, (CalleefptrStmt)stmt, grammar, singletons);
                break;
        }
        return out_pegraph;
    }

    private static void transfer_address(Pegraph pegraph, AllocStmt stmt, Grammar grammar, Singletons singletons) {
        // the KILL set
    ...
    private static void transfer_load(Pegraph pegraph, LoadStmt stmt, Grammar grammar, Singletons singletons) {
        // the KILL set
        Set<Integer> vertices_changed = new HashSet<>();
        Set<Integer> vertices_affected = new HashSet<>();
    ...
    private static void transfer_copy(Pegraph pegraph, AssignStmt stmt, Grammar grammar, Singletons singletons) {
        // the KILL set
        Set<Integer> vertices_changed = new HashSet<>();
        Set<Integer> vertices_affected = new HashSet<>();
    ...
    private static void transfer_phi(Pegraph pegraph, PhiStmt stmt, Grammar grammar, Singletons singletons) {
        // the KILL set
        Set<Integer> vertices_changed = new HashSet<>();
        Set<Integer> vertices_affected = new HashSet<>();
    ...
    private static void transfer_calleefptr(Pegraph pegraph, CalleefptrStmt stmt, Grammar grammar, Singletons singletons) {
    }

    private static void transfer_callfptr(Pegraph pegraph, CallfptrStmt stmt, Grammar grammar, Singletons singletons) {
    }

    private static void transfer_skip(Pegraph pegraph, SkipStmt stmt, Grammar grammar, Singletons singletons) {
    }

    private static void transfer_ret(Pegraph pegraph, RetStmt stmt, Grammar grammar, Singletons singletons) {
    }

    private static void transfer_return(Pegraph pegraph, ReturnStmt stmt, Grammar grammar, Singletons singletons) {
    }

    private static void transfer_call(Pegraph pegraph, CallStmt stmt, Grammar grammar, Singletons singletons) {
    }

    private static void peg_compute_add(Pegraph out, Stmt stmt, Grammar grammar) {
    ...
    private static void postProcessOneIteration(ComputationSet compset, boolean isDelete) {
        ...
        while (itNews.hasNext()) {
            ...
            int len = Tools.minusTwoArray(edges, labels,
    ...
    private static void getDirectAddedEdges(Pegraph out, Stmt stmt, Grammar grammar, Map<Integer, EdgeArray> m) {
        TYPE t = stmt.getType();
        if(t == TYPE.Alloca){
            AllocStmt stmt_alloc = (AllocStmt)stmt;
            getDirectAddedEdges_alloc(out, stmt_alloc, grammar, m);
        }
        else if(t == TYPE.Store){
            StoreStmt stmt_store = (StoreStmt)stmt;
            getDirectAddedEdges_store(out, stmt_store, grammar, m);
        }
        else if(t == TYPE.Load){
            LoadStmt stmt_load = (LoadStmt)stmt;
            getDirectAddedEdges_load(out, stmt_load, grammar, m);
        }
        else if(t == TYPE.Assign){
            AssignStmt stmt_assign = (AssignStmt)stmt;
    ...
    private static void getDirectAddedEdges_assign(Pegraph out, AssignStmt stmt, Grammar grammar, Map<Integer, EdgeArray> m) {
    ...
    private static void getDirectAddedEdges_load(Pegraph out, LoadStmt stmt, Grammar grammar, Map<Integer, EdgeArray> m) {
    ...
    private static void getDirectAddedEdges_store(Pegraph out, StoreStmt stmt, Grammar grammar, Map<Integer, EdgeArray> m) {
        //'a', '-a', 'd', '-d', and self-loop edges
        int src = stmt.getSrc();
        EdgeArray edges_src = new EdgeArray();
    ...
    private static void getDirectAddedEdges_alloc(Pegraph out, AllocStmt stmt, Grammar grammar, Map<Integer, EdgeArray> m) {
        //'a', '-a', 'd', '-d', and self-loop edges
        int src = stmt.getSrc();
        EdgeArray edges_src = new EdgeArray();
    ...
    private static void removeExistingEdges(EdgeArray edges_src, int src, Pegraph out, Map<Integer, EdgeArray> m) {
        ...
        int[] edges = new int[n1];
        byte[] labels = new byte[n1];
        int len = Tools.minusTwoArray(edges, labels, edges_src.getSize(),
    ...
    private static void getDirectAddedEdges_phi(Pegraph out, Stmt stmt, Grammar grammar, Map<Integer, EdgeArray> m) {
        PhiStmt stmt_phi = (PhiStmt)stmt;
        //'a', '-a', 'd', '-d', and self-loop edges
    ...
    private static void transfer_store(Pegraph out, StoreStmt stmt, Grammar grammar, Singletons singletons) {
        // the KILL set
        Set<Integer> vertices_changed = new HashSet<>();
        Set<Integer> vertices_affected = new HashSet<>();
    ...
    private static void strong_update_store_aux_simplify(int aux, int x, Pegraph out, Set<Integer> vertices_changed, Grammar grammar, Set<Integer> vertices_affected, Singletons singletons) {
        ...
        while (it.hasNext()) {
            ...
            if(deletedArray.getSize() != 0){
                ...
                int[] edges = new int[n1];
                byte[] labels = new byte[n1];
                int len = Tools.minusTwoArray(edges, labels, n1, out.getEdges(src), out.getLabels(src), n2, deletedArray.getEdges(), deletedArray.getLabels());
                if(len != 0){
                    out.setEdgeArray(src,len,edges,labels);
                }
                else{
    ...
    private static Pegraph getPartial(Stmt current, Stmt pred, Pegraph pred_graph, Grammar grammar, Map<Integer, NodeTuple> graphStore) {
        Pegraph out;
    ...
    private static Pegraph extractSubGraph_exit(Pegraph graph, int[] args, int len, int ret, Grammar grammar, Map<Integer, NodeTuple> graphStore) {
        if(len == 0 && ret == -1){
            return new Pegraph();
        }
        ...
        for (Map.Entry<Integer, NodeTuple> entry : graphStore.entrySet()) {
            Stmt stmt = entry.getValue().getStmt();
            if (stmt.getType() == TYPE.Call || stmt.getType() == TYPE.Callfptr) {
    ...
    public static Pegraph combine_synchronous(Stmt stmt, Grammar grammar, Map<Integer, NodeTuple> graphStore) {
        Pegraph out = null;
