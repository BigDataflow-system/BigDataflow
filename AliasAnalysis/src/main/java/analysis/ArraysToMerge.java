...
public class ArraysToMerge {
    ...
    public void addOneContainer() {
        if(arraySize == 0) {
            ...
            if(arraySize >= arrayCapacity) {
                arrayCapacity *= 2;
                index = Tools.myrealloc(index, arraySize, arrayCapacity);
                addr = Tools.myrealloc(addr, arraySize, arrayCapacity);
    ...
    public void addOneEdge(Integer edge, byte label) {
        if(arraySize > 0) {
            if(size >= capacity) {
                capacity *= 2;
                edges = Tools.myrealloc(edges,size,capacity);
                labels = Tools.myrealloc(labels,size,capacity);
    ...
    private void mergeKArrays() {
        if(size != 0) {
            ...
            int len = 0;
            if(arraySize > 1) {
                len = Tools.removeDuple(len,edge_v,edge_l,size,newEdges,newLabels);
            }
            else
                len = Tools.removeDuple(len,edge_v,edge_l,size,edges,labels);
